...
class Board {
  ...
  updateMaxTileIndex(){
    const maxTile = this.tiles.reduce((acc, curr) => {
      return acc.value > curr.value ? acc : curr;
    });
    const possibleNewIndex = Math.log2(maxTile.value)
    this.currentMaxTileIndex = possibleNewIndex
  }
}
